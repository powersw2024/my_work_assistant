<template>
  <div class="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-opacity duration-300">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-6xl mx-4 max-h-[90vh] flex flex-col transform transition-all duration-300 scale-100 opacity-100">
      <div class="px-6 py-5 border-b border-surface-200 flex justify-between items-center">
        ...
        <button @click="$emit('close')" class="text-gray-400 hover:text-rose-500 hover:bg-rose-50 p-2 rounded-xl transition-colors">
      ...
      <div class="flex-1 overflow-y-auto custom-scrollbar bg-surface-50/50">
        <div class="p-6 space-y-8">
              ...
              <table class="table">
                ...
                <tbody>
                  <tr v-for="(person, index) in settings.personnel" :key="'person-'+index" class="group">
                    ...
                    <td class="text-right">
                      <button @click="removePerson(index)" class="p-1.5 text-gray-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors opacity-0 group-hover:opacity-100" title="删除">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div class="bg-surface-50/50 rounded-xl p-5 border border-surface-200">
                ...
                <div class="table-container">
                  <table class="table">
                    ...
                    <tbody>
                      <tr v-for="(category, index) in settings.expense_categories.mainCategories" :key="'main-'+index" class="group">
                        <td><span class="tag-gray">{{ category }}</span></td>
                        <td class="text-right">
                          <button @click="removeMainCategory(index)" class="p-1 text-gray-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors opacity-0 group-hover:opacity-100" title="删除">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                            </svg>
                          </button>
              ...
              <div class="bg-surface-50/50 rounded-xl p-5 border border-surface-200">
                ...
                <div class="flex flex-col sm:flex-row gap-3 mb-5">
                  ...
                  />
                  <button @click="addSubCategory" :disabled="!selectedMainCategory" class="btn btn-success whitespace-nowrap">
                    添加
                  </button>
                </div>
                  ...
                  <div class="table-container">
                    <table class="table">
                      <tbody class="divide-y divide-surface-100">
                        <tr v-for="(subcategory, index) in settings.expense_categories.categoriesByParent[selectedMainCategory] || []" :key="'sub-'+index" class="group">
                          <td class="py-3 px-4"><span class="text-gray-700 font-medium text-sm">{{ subcategory }}</span></td>
                          <td class="py-3 px-4 text-right">
                            <button @click="removeSubCategory(selectedMainCategory, index)" class="p-1 text-gray-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors opacity-0 group-hover:opacity-100" title="删除">
                              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                              </svg>
                            </button>
                        ...
                        <tr v-if="!(settings.expense_categories.categoriesByParent[selectedMainCategory] && settings.expense_categories.categoriesByParent[selectedMainCategory].length > 0)">
                ...
                <div v-else class="mt-6 text-center p-6 border border-dashed border-surface-300 rounded-xl text-gray-400 text-sm">
                  请先在上方选择一个主类别
                </div>
              </div>
            </div>
          ...
          <section class="bg-white rounded-2xl p-6 shadow-sm border border-surface-200">
            ...
            <div class="flex flex-col sm:flex-row gap-3 mb-6">
              ...
              />
              <button @click="addVoucherType" class="btn bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm hover:shadow">
                添加凭证类型
              </button>
            </div>
              ...
              <table class="table">
                ...
                <tbody>
                  <tr v-for="(type, index) in settings.voucher_types" :key="'voucher-'+index" class="group">
                    ...
                    <td class="text-right">
                      <button @click="removeVoucherType(index)" class="p-1.5 text-gray-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors opacity-0 group-hover:opacity-100" title="删除">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                        </svg>
                      </button>
